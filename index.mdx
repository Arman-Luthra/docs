---
title: "Skop API"
description: "AI-powered document discovery and extraction from websites"
---

## What is Skop?

Skop is an intelligent PDF scraper API that uses AI to find and extract documents from websites based on natural language prompts. Simply provide a website URL and describe what documents you're looking for, and Skop will navigate the site, identify relevant documents, and extract them for you.

**Base URL:** `https://api.skop.dev`

<Card title="Get Started" icon="rocket" horizontal href="/quickstart">
  Start scraping documents in minutes with our quickstart guide.
</Card>

## Key Features

<CardGroup cols={2}>
  <Card title="Natural Language Prompts" icon="comments">
    Describe what you're looking for in plain English: "Find board meeting minutes from 2025"
  </Card>
  <Card title="Smart Document Discovery" icon="magnifying-glass">
    AI-powered navigation finds documents across multiple pages automatically
  </Card>
  <Card title="Multiple File Formats" icon="file-lines">
    Extract PDF, DOC, DOCX, TXT and other document formats
  </Card>
  <Card title="Confidence Scoring" icon="chart-line">
    Each extracted document includes an AI confidence score for relevance
  </Card>
</CardGroup>

## Common Use Cases

<Columns cols={2}>
  <Card title="Financial Reports" icon="chart-pie">
    Extract quarterly reports, annual statements, and financial documents
  </Card>
  <Card title="Legal Documents" icon="scale-balanced">
    Find contracts, policies, terms of service, and legal notices
  </Card>
  <Card title="Meeting Minutes" icon="users">
    Gather board meeting minutes, committee reports, and corporate governance documents
  </Card>
  <Card title="Research Papers" icon="flask">
    Collect academic papers, whitepapers, and technical documentation
  </Card>
</Columns>

## Quick Example

```javascript
const response = await fetch('https://api.skop.dev/scrape/', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer sk-your-api-key',
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    website: 'https://example.com',
    prompt: 'Find meeting minutes from 2025'
  })
})

const job = await response.json()
console.log('Job created:', job.job_id)
```

## API Reference

<Card title="Full API Documentation" icon="code" href="/api-reference/introduction">
  Explore all endpoints, parameters, and response formats
</Card>